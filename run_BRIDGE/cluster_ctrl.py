...
[prot_count_array,prot_cells,adt_names] = pa.import_prot_data('8309714/GSM3596096_ctrl-ADT-count.csv')
vlm = vcy.VelocytoLoom("8309714/190426_ctrl.loom")
...
cluster_labels = ['CD4+ T','B/Mono.','NK','CD8+ T']
vlm.COLORS = COLORS
...
df.to_csv("8309714/data_filtered_ctrl/U.csv")

df = pd.DataFrame(vlm.S, index=gene_names, columns=CellIDs)
df.to_csv("8309714/data_filtered_ctrl/S.csv")

df = pd.DataFrame(prot_count_array, index=adt_names, columns=CellIDs)
df.to_csv("8309714/data_filtered_ctrl/P.csv")

cluster_ID
len(cluster_ID)
np.savetxt("8309714/data_filtered_ctrl/cluster_ID.txt",cluster_ID,fmt="%.0f",delimiter="\t")
